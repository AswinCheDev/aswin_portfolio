...
const cardGLB = '/assests/lanyard/Compressed/card.glb';
...
function Band({
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current) {
      ...
      card.current.setAngvel({ x: ang.x, y: ang.y - rot.y * 0.25, z: ang.z - rot.z * 0.25 });
    }
  });
    ...
    <>
      <group position={[isMobile ? 0 : -1.0, 4, 0]}>
        ...
        <RigidBody position={[2, 0, 0]} ref={card} {...(segmentProps as any)} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          <CuboidCollider args={[0.56, 0.7875, 0.007]} />
          <group
            scale={1.575}
